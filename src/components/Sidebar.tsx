...
import { MessageCircle, LayoutDashboard, Bot, Plug, Settings, LogOut, X, Plus, Users, Newspaper } from 'lucide-react'
...
export default function Sidebar({ onClose, onLogout }: SidebarProps) {
  ...
  return <div className="sidebar w-72 h-full flex flex-col bg-white dark:bg-dark-900 border-e border-dark-200 dark:border-dark-700">
    <div className="flex items-center justify-between px-6 py-5 border-b border-dark-200 dark:border-dark-700"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-primary-600 to-accent-600 flex items-center justify-center"><span className="text-white font-bold text-xl">M</span></div><div><div className="font-semibold text-lg">{t('common.brand')}</div><div className="text-[10px] text-dark-500 -mt-1">AI Workspace</div></div></div><button onClick={onClose} aria-label={t('common.close')} className="md:hidden p-2 text-dark-400"><X size={18} /></button></div>
