...
describe('magic-link redirect', () => {
  ...
  it('uses an explicit HTTPS origin and strips paths', () => {
    expect(resolveAuthRedirectUrl('https://moatazasaif.vercel.app/', 'https://preview.example.com')).toBe('https://preview.example.com/login')
    expect(resolveAuthRedirectUrl(undefined, 'https://custom.example.com/dashboard')).toBe('https://custom.example.com/login')
    expect(resolveAuthRedirectUrl('https://custom.example.com/dashboard', 'https://moatazasaif.vercel.app')).toBe('https://moatazasaif.vercel.app/login')
  })
})
