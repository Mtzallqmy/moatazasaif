...
export default function Integrations() {
  ...
  const [form, setForm] = useState({ name: '', botToken: '', providerId: '', model: '' })
  const [testedBot, setTestedBot] = useState<{ botId: string; botUsername?: string; botFirstName?: string } | null>(null)
  ...
  const testToken = async () => {
    ...
    setBusy('test-token')
    try {
      const result = await apiJson<{ botId: string; botUsername?: string; botFirstName?: string; message: string }>('/api/integrations/telegram/test', {
        method: 'POST', headers: await authHeaders(), body: JSON.stringify({ botToken: form.botToken }),
      })
  ...
  const addIntegration = async () => {
    ...
    try {
      ...
      setIntegrations((current) => [result.integration, ...current]); setForm((current) => ({ ...current, name: '', botToken: '', model: '' })); setTestedBot(null)
      toast.success('تم تسجيل Webhook والتحقق منه عبر Telegram فعليًا')
  ...
  return <div className="p-6 max-w-6xl mx-auto">
    <div className="mb-8"><h1 className="text-3xl font-semibold tracking-tight">التكاملات</h1><p className="text-dark-400 mt-1">Telegram يعمل من الخادم حتى عند إغلاق المتصفح. يجب اختيار مزود محفوظ ومختبر؛ مفاتيح الجلسة المؤقتة لا تصلح للـ Webhook.</p></div>
