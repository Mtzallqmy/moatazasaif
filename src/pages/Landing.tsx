  ...
  return <div className="app-canvas min-h-screen overflow-hidden">
    ...
    <nav className="sticky top-0 z-50 bg-white/80 dark:bg-dark-950/80 backdrop-blur-xl border-b border-dark-200 dark:border-white/10">
      <div className="max-w-7xl mx-auto px-5 pe-20 sm:pe-24 h-20 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-primary-500 to-accent-600 text-white grid place-items-center font-bold text-xl">M</div><div><div className="font-semibold text-lg leading-tight">{t('common.brand')}</div><div className="text-[10px] text-dark-500">AI Workspace</div></div></Link>
        <div className="flex items-center gap-2 sm:gap-4 text-sm"><Link to="/blog" className="text-dark-500 hover:text-dark-900 dark:hover:text-white hidden md:block">{tr('المقالات', 'Articles')}</Link><Link to="/privacy" className="text-dark-500 hover:text-dark-900 dark:hover:text-white hidden lg:block">{t('landing.privacy')}</Link><Link to="/login" className="btn btn-secondary hidden sm:inline-flex">{t('landing.account')}</Link><Link to="/register" className="btn btn-primary">{t('landing.register')}</Link></div>
      </div>
    </nav>
    ...
    <main>
      <section className="relative px-5 pt-20 sm:pt-28 pb-20">
        ...
        <div className="max-w-6xl mx-auto relative z-10 text-center">
          ...
          <p className="max-w-3xl mx-auto text-lg sm:text-xl text-dark-500 mt-7 leading-8">{t('landing.subtitle')}</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-10"><Link to="/chat" className="btn btn-primary px-8 py-4 text-base group">{t('landing.try')} <ArrowLeft className="group-hover:-translate-x-1 transition-transform" size={18} /></Link><Link to="/blog" className="btn btn-secondary px-8 py-4 text-base"><BookOpen size={18} />{tr('استكشف المقالات', 'Explore articles')}</Link></div>
