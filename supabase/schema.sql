...
-- =========================================================
-- Private chat files and editable project workspace
-- =========================================================
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('chat-files', 'chat-files', false, 3145728, array[
  'image/png','image/jpeg','image/webp','text/plain','text/markdown','application/json',
  'text/csv','text/tab-separated-values','application/xml','text/xml','application/yaml',
  'text/yaml','application/x-yaml','application/sql','text/javascript','application/javascript',
  'text/typescript','application/typescript','text/x-python','text/html','text/css','text/x-shellscript'
]::text[])
...
create table if not exists public.chat_files (
  ...
  original_name text not null check (char_length(original_name) between 1 and 200 and original_name !~ '[\x00-\x1F\x7F]'),
  ...
  created_at timestamptz not null default now(),
  check (storage_path like user_id::text || '/' || chat_id::text || '/%' and storage_path !~ '(^|/)\.\.(/|$)')
);
create index if not exists chat_files_chat_created_idx on public.chat_files(chat_id, created_at);
create index if not exists chat_files_user_created_idx on public.chat_files(user_id, created_at desc);
...
create table if not exists public.projects (
  ...
  template text not null default 'empty' check (template in ('empty','vite-react','node-api','python')),
...
create table if not exists public.project_files (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  path text not null check (char_length(path) between 1 and 240 and path !~ '^/' and path !~ '(^|/)\.\.(/|$)' and path !~ '[\x00-\x1F\x7F]'),
...
revoke all on public.chat_files, public.projects, public.project_files from public, anon, authenticated;
grant all on public.chat_files, public.projects, public.project_files to service_role;

drop policy if exists chat_files_owner_all on public.chat_files;
create policy chat_files_owner_all on public.chat_files for all to authenticated
...
create policy project_files_owner_all on public.project_files for all to authenticated
  using ((select auth.uid()) = user_id and exists (select 1 from public.projects project where project.id = project_id and project.user_id = (select auth.uid())))
  with check ((select auth.uid()) = user_id and exists (select 1 from public.projects project where project.id = project_id and project.user_id = (select auth.uid())));
